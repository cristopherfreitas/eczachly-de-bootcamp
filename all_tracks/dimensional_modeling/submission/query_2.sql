...
-- v2, window functions and row after
WITH last_year AS (
    SELECT *
    FROM cristophersfr.actors
    WHERE current_year = 1934
...
cumulative_table AS (
    SELECT
        ...
        CASE
            ...
            -- If the actor has no history, create an array with only the new one.
            WHEN ly.films IS NULL AND ty.film IS NOT NULL THEN
                array_agg(ROW(ty.film, ty.film_id, ty.votes, ty.rating)) OVER (PARTITION BY ty.actor_id)
            -- If the actor has both history and new films, append the new ones.
            WHEN ty.film IS NOT NULL AND ly.films IS NOT NULL THEN
                array_agg(ROW(ty.film, ty.film_id, ty.votes, ty.rating)) OVER (PARTITION BY ty.actor_id) || ly.films
        END AS films,
        -- The quality class is the average rating of the actor's films.
        CASE
            WHEN AVG(rating) OVER (PARTITION BY ty.actor_id) > 8 THEN 'star'
            WHEN AVG(rating) OVER (PARTITION BY ty.actor_id) > 7 THEN 'good'
            WHEN AVG(rating) OVER (PARTITION BY ty.actor_id) > 6 THEN 'average'
            ELSE 'bad'
        END AS quality_class,
        ty.year is not null as is_active,
        COALESCE(ty.year, ly.current_year + 1) as current_year
